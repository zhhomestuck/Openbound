...
xhr.onabort=function(){Modernizr.addTest('xhrarraybuffer',function(){return false;});};xhr.onerror=function(){Modernizr.addTest('xhrarraybuffer',function(){return false;});};xhr.send();Sburb.firedAsync=true;}else{Modernizr.addTest('xhrblob',function(){return false;});Modernizr.addTest('xhrarraybuffer',function(){return false;});}
if(!('xhrblob'in Modernizr&&'xhrarraybuffer'in Modernizr&&'datauri'in Modernizr)){console.log("Still waiting for Modernizr to load...");setTimeout(function(){Sburb.initialize(div,levelName,includeDevTools);},200);Sburb.crashed=true;return;}
...
return;Sburb.debugger=new Sburb.Debugger();var deploy='   \
 <div style="position: relative;\
  ...
  </br>';if(includeDevTools){Sburb._include_dev=true;}
deploy+='</div>';document.getElementById(div).innerHTML=deploy;var gameDiv=document.getElementById("SBURBgameDiv");gameDiv.onkeydown=_onkeydown;gameDiv.onkeyup=_onkeyup;Sburb.Stage=document.getElementById("SBURBStage");Sburb.Stage.scaleX=Sburb.Stage.scaleY=3;Sburb.Stage.x=Sburb.Stage.y=0;Sburb.Stage.fps=30;Sburb.Stage.fade=0;Sburb.Stage.fadeRate=0.1;Sburb.stage=Sburb.Stage.getContext("2d");Sburb.Stage.onblur=_onblur;Sburb.chooser=new Sburb.Chooser();Sburb.dialoger=null;Sburb.assetManager=new Sburb.AssetManager();Sburb.assets=Sburb.assetManager.assets;Sburb.rooms={};Sburb.sprites={};Sburb.effects={};Sburb.buttons={};Sburb.hud={};Sburb.gameState={};Sburb.pressed={};Sburb.pressedOrder=[];Sburb.loadSerialFromXML(levelName);}
...
var _onkeydown=function(e){if(Sburb.updateLoop){if(Sburb.chooser.choosing){if(e.keyCode==Sburb.Keys.down||e.keyCode==Sburb.Keys.s){Sburb.chooser.nextChoice();}
...
if(Sburb.chooser.choices.length>0){Sburb.chooser.choices.push(new Sburb.Action("cancel","cancel","取消。"));beginChoosing();}}}}
...
if(hasControl()){Sburb.char.handleInputs(Sburb.pressed,Sburb.pressedOrder);}else{Sburb.char.moveNone();}
...
if(((Sburb.curAction&&Sburb.curAction.followUp!=action&&Sburb.curAction!=action)||!hasControl())&&action.soft){return;}
...
Sburb.Character.prototype.handleInputs=function(pressed,order){var down=-1,up=-1,left=-1,right=-1,most=0;down=Math.max(order.indexOf(Sburb.Keys.down),order.indexOf(Sburb.Keys.s));up=Math.max(order.indexOf(Sburb.Keys.up),order.indexOf(Sburb.Keys.w));left=Math.max(order.indexOf(Sburb.Keys.left),order.indexOf(Sburb.Keys.a));right=Math.max(order.indexOf(Sburb.Keys.right),order.indexOf(Sburb.Keys.d));most=Math.max(down,up,left,right,most);if(down==most){this.moveDown();}else if(up==most){this.moveUp();}else if(left==most){this.moveLeft();}else if(right==most){this.moveRight();}else{this.moveNone();}
this.handledInput=2;}
...
return newButton;}
return Sburb;})(Sburb||{});var Sburb=(function(Sburb){Sburb.Animation=function(name,sheet,x,y,colSize,rowSize,startPos,length,frameInterval,loopNum,followUp,flipX,flipY,sliced,numCols,numRows){this.sheet=sheet;this.sliced=sliced?true:false;this.x=x;this.y=y;this.rowSize=rowSize;this.colSize=colSize;this.startPos=startPos;this.length=length;this.curInterval=0;this.curFrame=0;this.name=name;this.loopNum=typeof loopNum=="number"?loopNum:-1;this.curLoop=0;this.followUp=followUp;this.flipX=flipX?true:false;this.flipY=flipY?true:false;if(sliced){this.numRows=numRows;this.numCols=numCols;this.sheets={};for(var colNum=0;colNum<this.numCols;colNum++){for(var rowNum=0;rowNum<this.numRows;rowNum++){var sheet=Sburb.assets[this.sheet+"_"+colNum+"_"+rowNum];if(sheet){if(!this.sheets[colNum]){this.sheets[colNum]={};}
this.sheets[colNum][rowNum]=sheet;}}}
this.draw=this.drawSliced;}else{this.numRows=sheet.height/rowSize;this.numCols=sheet.width/colSize;this.draw=this.drawNormal;}
...
Sburb.FontEngine.prototype.parseEverything=function(){this.formatQueue=[];if(this.formatted){this.escaped={};this.parsePrefixes();this.parseEscapes();this.parseText(forceNewLine=true);this.parseUnderlines();this.parseColors();}}
Sburb.FontEngine.prototype.parseText=function(forceNewLine=false){Sburb.stage.font=this.font;this.lines=[];var i=0;var lastSpace=0;var lineStart=0;
for(i=0;i<this.text.length;i++){if(this.text.charAt(i)==" "){lastSpace=i;}else if(this.text.charAt(i)=="\n"){this.lines.push(this.text.substring(lineStart,i));lineStart=i+1;lastSpace=lineStart;continue;}
if(Sburb.stage.measureText(this.text.substring(lineStart,i+1)).width>this.width){if(lineStart==lastSpace){this.lines.push(this.text.substring(lineStart,i));lineStart=i;lastSpace=i;}
else if(forceNewLine&&this.text.charAt(i+1).match(/[^( -~)]/)){this.text=[this.text.slice(0, i), '\n', this.text.slice(i)].join('');
this.lines.push(this.text.substring(lineStart,i));lineStart=i+1;lastSpace=lineStart;}else{this.lines.push(this.text.substring(lineStart,lastSpace));lineStart=lastSpace+1;lastSpace=lineStart;}}}
...
Sburb.FontEngine.prototype.draw=function(){var i=0;var lenCount=0;var linePos=0;var strStart,strEnd;var currentFormat=0;var currentFormats=[];var nextStop;var curLine;var offsetX=0;while(i<Math.floor(this.height/this.lineHeight)&&i<this.lines.length){Sburb.stage.save();Sburb.stage.textBaseline="top";Sburb.stage.textAlign=this.align;curLine=this.lines[i];var curFont=this.font;var curColor=this.color;var underlining=false;nextStop=curLine.length;if(currentFormat<this.formatQueue.length&&this.formatQueue[currentFormat].minIndex<=lenCount+linePos){currentFormats.push(this.formatQueue[currentFormat]);currentFormat++;}
...
linePos=-1;}
var numChars=strEnd-strStart;if(numChars>0){var startX=this.x+offsetX;var startY=this.y+i*this.lineHeight;var numCharsWidth=Sburb.stage.measureText(curLine.substring(strStart,strEnd)).width;
Sburb.stage.font=curFont;Sburb.stage.strokeStyle=Sburb.stage.fillStyle=curColor;Sburb.stage.fillText(curLine.substring(strStart,strEnd),startX,startY);offsetX+=numCharsWidth;if(underlining&&strStart<strEnd){if(Sburb.stage.lineWidth!=0.6){Sburb.stage.lineWidth=0.6;}
if(Sburb.stage.lineCap!="square"){Sburb.stage.lineCap="square";}
Sburb.stage.beginPath();Sburb.stage.moveTo(startX,startY+this.lineHeight-3);Sburb.stage.lineTo(startX+numCharsWidth,startY+this.lineHeight-3);Sburb.stage.closePath();Sburb.stage.stroke();}}if(linePos==-1){lenCount+=this.lines[i].length+1;linePos=0;offsetX=0;i++;}
Sburb.stage.restore();}}
...
(this.times!=1?"' times='"+this.times:"")+"'>");output=output.concat('<args>'+escape(this.info.trim())+'</args>');if(this.followUp){output=this.followUp.serialize(output);}
...
var events={};events.spriteProperty=function(info){var params=parseParams(info);var token;var query=params[2];if(query.indexOf(">")>-1||query.indexOf("GREATER")>-1){token=">";this.trigger=function(entity,property,target){return entity[property]>target;};}else if(query.indexOf("<")>-1||query.indexOf("LESS")>-1){token="<";this.trigger=function(entity,property,target){return entity[property]<target;};}else if(query.indexOf("!=")>-1){token="!=";this.trigger=function(entity,property,target){return entity[property]!=target;};}else if(query.indexOf("=")>-1){token="=";this.trigger=function(entity,property,target){return entity[property]==target;};}
...
this.checkCompletion=function(){this.time--;return this.time<=0;};};events.played=function(info){var params=parseParams(info);this.reset=function(){this.entity=Sburb.sprites[params[1]];}
this.checkCompletion=function(){var entity=this.entity;if(this.entity=="char"){entity=Sburb.char;}
...
return false;}};events.gameState=function(info){var params=parseParams(info);var token;var query=params[1];if(query.indexOf(">")>-1||query.indexOf("GREATER")>-1){token=">";this.trigger=function(property,target){return Sburb.gameState[property]>target;};}else if(query.indexOf("<")>-1||query.indexOf("LESS")>-1){token="<";this.trigger=function(property,target){return Sburb.gameState[property]<target;};}else if(query.indexOf("!=")>-1){token="!=";this.trigger=function(property,target){return Sburb.gameState[property]!=target;};}else if(query.indexOf("=")>-1){token="=";this.trigger=function(property,target){return Sburb.gameState[property]==target;};}
...
events.noActions=function(info){this.reset=function(){}
this.checkCompletion=function(){return Sburb.curAction==null;}}
...
Sburb.events=events;return Sburb;})(Sburb||{});var Sburb=(function(Sburb){Sburb.Trigger=function(info,action,followUp,restart,detonate){if(typeof info=="string"){info=[info];}
this.info=info;this.followUp=followUp?followUp:null;this.action=action?action:null;this.restart=restart?restart:false;this.detonate=detonate?detonate:false;this.events=[];for(var i=0;i<info.length;i++){var inf=this.info[i].trim();var params=inf.split(",");var type=params[0];this.events[i]=new Sburb.events[type](inf);}
this.reset();}
Sburb.Trigger.prototype.reset=function(){for(var i=0;i<this.events.length;i++){this.events[i].reset();}}
Sburb.Trigger.prototype.checkCompletion=function(){var result=true;for(var i=0;i<this.events.length;i++){result=result&&this.events[i].checkCompletion();}
return result;}
...
(this.detonate?" detonate='true'":"")+">");for(var i=0;i<this.info.length;i++){output=output.concat("<args>"+escape(this.info[i])+"</args>");}
...
var actions=triggerNode.getElementsByTagName("action");var action=null;var restart=false;var detonate=false;if(actions.length>0&&actions[0].parentNode==triggerNode){action=Sburb.parseAction(actions[0]);}
...
commands.changeChar=function(info){Sburb.char.becomeNPC();Sburb.char.walk();Sburb.destFocus=Sburb.char=Sburb.sprites[info];Sburb.char.becomePlayer();Sburb.setCurRoomOf(Sburb.char);}
...
if(Sburb.isStateInStorage(true,local)){actions.push(new Sburb.Action("load","true, "+local,"讀檔: "+Sburb.getStateDescription(true)));}
if(Sburb.tests.storage){actions.push(new Sburb.Action("save","false,"+local,"存檔"));}
actions.push(new Sburb.Action("cancel",null,"取消"));Sburb.chooser.choices=actions;Sburb.chooser.beginChoosing(Sburb.Stage.x+20,Sburb.Stage.y+50);}
commands.setGameState=function(info){var params=parseParams(info);Sburb.gameState[params[0]]=params[1];}
commands.goBack=function(info){var params=parseParams(info);var character=parseCharacterString(params[0]);var vx=0;vy=0;if(character.facing=="Front"){vx=0;vy=-character.speed;}else if(character.facing=="Back"){vx=0;vy=character.speed;}else if(character.facing=="Left"){vx=character.speed;vy=0;}else if(character.facing=="Right"){vx=-character.speed;vy=0;}
...
(loadedFilesExist?("' loadedFiles='"+loadedFiles):"")+"'>\n";output=serializeAssets(output,assets,effects);output=serializeTemplates(output,templateClasses);output=serializeHud(output,hud,dialoger);output=serializeLooseObjects(output,rooms,sprites);output=serializeRooms(output,rooms);output=serializeGameState(output,gameState);output=output.concat("\n</sburb>");if(out){out.value=output;}
return output;}
...
return false;var decoded=Iuppiter.decompress(Iuppiter.Base64.decode(Iuppiter.toByteArray(compressed),true));Sburb.loadSerial(decoded);return true;}
...
{var srcVal=curAsset.originalVals[i];innerHTML+=(firstSrc?";":"")+srcVal;firstSrc=true;}}else if(curAsset.type=="path"){for(var i=0;i<curAsset.points.length;i++){innerHTML=innerHTML.concat(curAsset.points[i].x+","+curAsset.points[i].y);if(i!=curAsset.points.length-1){innerHTML=innerHTML.concat(";");}}}else if(curAsset.type=="movie"){innerHTML+=curAsset.originalVals;}else if(curAsset.type=="font"){innerHTML+=curAsset.originalVals;}
...
document.getElementById("SBURBmovieBin").innerHTML="";document.getElementById("SBURBfontBin").innerHTML="";Sburb.gameState={};Sburb.globalVolume=1;Sburb.hud={};Sburb.sprites={};Sburb.buttons={};Sburb.effects={};Sburb.curAction=null;Sburb.pressed={};Sburb.pressedOrder=[];Sburb.chooser=new Sburb.Chooser();Sburb.dialoger=null;Sburb.curRoom=null;Sburb.char=null;Sburb.assetManager.resourcePath="";Sburb.assetManager.levelPath="";Sburb.loadedFiles={};}
...
var request=new XMLHttpRequest();request.open('GET',file,false);try{request.send(null);}catch(err){console.log("If you are running Google Chrome, you need to run it with the -allow-file-access-from-files switch to load this.");fi=document.getElementById("levelFile");return;}
...
newAsset=Sburb.createPathAsset(name,path);}else if(type=="movie"){newAsset=Sburb.createMovieAsset(name,value,blobUrls);}else if(type=="font"){newAsset=Sburb.createFontAsset(name,value);}
...
while(loadQueue.length>0){var input=loadQueue[0];loadQueue.splice(0,1);parseTemplateClasses(input);applyTemplateClasses(input);parseButtons(input);parseSprites(input);parseCharacters(input);parseFighters(input);parseRooms(input);parseGameState(input);parseHud(input);parseEffects(input);parseState(input);}
...
Sburb.Dialoger.prototype.nextDialog=function(){var nextDialog=this.queue.pop().trim();this.dialog.setText(nextDialog);this.dialog.showSubText(0,0);var prefix=nextDialog.split(" ",1)[0];if(prefix.indexOf("~")>=0&&prefix.indexOf("/")!=prefix.indexOf("~")-1){var firstIndex=prefix.indexOf("~");var lastIndex=prefix.length;var ampIndex=prefix.indexOf("%");if(ampIndex>firstIndex){lastIndex=ampIndex;}
...
Sburb.Chooser.prototype.beginChoosing=function(x,y){var width=this.minWidth;var height=0;var basis=new Sburb.FontEngine();for(i=0;i<this.choices.length;i++){width=Math.max(width,Sburb.stage.measureText(this.choices[i].name).width+basis.charWidth*3);}
...
Sburb.Chooser.prototype.draw=function(){if(this.choosing){Sburb.stage.save();var x,y,width=this.minWidth,height=0,i;x=this.dialogs[0].x;y=this.dialogs[0].y-1;for(i=0;i<this.dialogs.length;i++){width=Math.max(width,Sburb.stage.measureText(this.dialogs[i].lines[0]).width+this.dialogs[i].charWidth);}
...
return ret}
Sburb.createFontAsset=function(name,sources){var ret={font:sources[0]};ret.name=name;ret.originalVals=sources;ret.type="font";ret.done=function(url){ret.loaded=true;};ret.failure=function(){ret.failed=true;};ret.success=function(url,id){var font="url('"+url+"') format('"+ret.sources[id]+"')";ret.sources[id]=font;ret.remaining-=1;if(!ret.remaining){document.getElementById("SBURBfontBin").innerHTML+='<style type="text/css">@font-face{ font-family: '+ret.name+'; src: '+ret.sources.join(',')+'; '+ret.extra+'}</style>';Sburb.stage.font="10px "+name;ret.done();}};ret.assetOnLoadFunction=function(fn){if(ret.loaded){if(fn){fn();}
...
if(this.open){var lineWidth=Sburb.stage.lineWidth;var strokeStyle=Sburb.stage.strokeStyle;Sburb.stage.lineWidth=9;Sburb.stage.strokeStyle="#000000";Sburb.stage.beginPath();for(var i=0;i<=450;i+=10){Sburb.stage.moveTo(0,i);Sburb.stage.lineTo(650,i);}
Sburb.stage.closePath();Sburb.stage.stroke();Sburb.stage.lineWidth=lineWidth;Sburb.stage.strokeStyle=strokeStyle;Sburb.stage.textAlign="left";Sburb.stage.font="bold 18px Verdana";for(var error=this.errors.length-1,y=435;error>=0;error--,y-=24){if(this.errors[error].type in this.colors){Sburb.stage.fillStyle=this.colors[this.errors[error].type];}else{Sburb.stage.fillStyle="#000000";}
Sburb.stage.fillText(this.errors[error].text,10,y);}
var y=70;Sburb.stage.textAlign="right";Sburb.stage.font="bold 18px Verdana";for(var test in this.tests){if(!this.tests.hasOwnProperty(test))continue;var color=this.tests[test];var width=Sburb.stage.measureText(test).width;Sburb.stage.fillStyle=this.colors[color];Sburb.stage.fillRect(630-width,y,width+10,24);Sburb.stage.fillStyle="#FFFFFF";Sburb.stage.fillText(test,635,y+18);y+=26;}
Sburb.stage.fillStyle="#000000";Sburb.stage.fillRect(200,10,250,65);Sburb.stage.textAlign="center";Sburb.stage.fillStyle="#FFFFFF";Sburb.stage.font="bold 28px Verdana";Sburb.stage.fillText("Sburb Debugger",325,33);Sburb.stage.font="14px Verdana";Sburb.stage.fillText("Press SPACE to send bug report",325,51);Sburb.stage.fillText("Press ~ or ESC to exit",325,67);Sburb.stage.fillStyle="#000000";Sburb.stage.fillRect(600,425,50,25);Sburb.stage.textAlign="right";Sburb.stage.fillStyle="#FFFFFF";Sburb.stage.font="bold 16px Verdana";Sburb.stage.fillText(this.fps+"fps",642,440);Sburb.stage.textAlign="center";Sburb.stage.fillStyle="#000000";}}
Sburb.Debugger.prototype.sendDebugReport=function(){var div=document.createElement('div');var title=document.createElement('h2');var form=document.createElement('form');var textbox=document.createElement('textarea');var submit=document.createElement('input');div.id="sburb_debug_report";div.style.position="absolute";div.style.zIndex="9999";div.style.background="white";div.style.width="550px";div.style.top="100px";div.style.left="50px";title.innerText="What seems to be the problem?";title.style.margin="0 20px";form.method="POST";form.action="#";form.style.textAlign="right";form.onsubmit=this.submitForm;textbox.id="sburb_debug_report_box";textbox.style.width="530px";textbox.style.height="200px";textbox.style.margin="0 8px";textbox.style.display="block";submit.type="submit";submit.value="Send Bug Report";form.appendChild(textbox);form.appendChild(submit);div.appendChild(title);div.appendChild(form);document.getElementById("SBURBgameDiv").parentNode.appendChild(div);textbox.focus();}
